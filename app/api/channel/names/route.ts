...
import { nameToolSchema, namesOnlySchema } from "@/lib/validation/schemas";
import { CHANNEL_NAMES_PROMPT } from "@/lib/ai/prompts";
...
export async function POST(req: NextRequest) {
  ...
  const parsed = nameToolSchema.safeParse(await req.json().catch(() => null));
  ...
  try {
    const result = await runStructuredText({
      system: CHANNEL_NAMES_PROMPT.system,
      user: CHANNEL_NAMES_PROMPT.user(parsed.data.niche, parsed.data.personality),
      schema: namesOnlySchema,
    ...
    return fail("GENERATION_FAILED", "Something went wrong while generating names. Please try again.", 500);
  }
}
